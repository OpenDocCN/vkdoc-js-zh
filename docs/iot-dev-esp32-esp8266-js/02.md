...
在 JavaScript 中描述字符串的另一种方法是使用反斜杠字符(```)。以这种方式定义的字符串被称为*模板文字*，并且有几个有用的属性，包括它们可以跨越多行(潜在地使你的字符串更可读；将清单 [2-10](#PC35) 与之前的示例进行比较。
...
创建小整数数组是很常见的。在 C 和 C++中，可以很容易地在堆栈上声明静态数组。
