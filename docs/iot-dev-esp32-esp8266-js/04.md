...
当 BLE 栈准备好使用时，以及当到中心的连接丢失时，`hrm`示例调用`startAdvertising`。当`startAdvertising`被调用时，外设广播它的广告数据类型标志值、它的名称和它的服务(心率和电池)，如清单 [4-6](#PC12) 所示。心率和电池服务的 UUIDs 来自 GATT 规范。
