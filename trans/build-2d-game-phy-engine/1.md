...
2.  在新建项目窗口中，在类别部分选择 HTML5，在项目部分选择 HTML5 应用，如图 [1-3](#Fig3) 所示。单击“下一步”按钮，打开项目配置窗口。
...
NetBeans 将为您生成一个简单而完整的 HTML5 应用项目的模板。您的 IDE 看起来应该类似于图 [1-5](#Fig5) 。
...
导航到文件系统上的 html 5 应用项目位置，例如使用 Windows 中的 Explorer OS 实用程序。您可以看到，在项目文件夹中，NetBeans 生成了 nbProject、public_html 和 test 文件夹。表 [1-1](#Tab1) 总结了这些文件夹和 index.html 文件的用途。
