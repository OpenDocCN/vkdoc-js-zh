...
![image](Images/sq.jpg) **注**我们在整章中都使用了我们在前一章中创建的 DOMhelp 库(实际上，在本书的其余部分也是如此)。
...
![image](Images/sq.jpg) **注**耐心是关键。addEvent()和 cancelClick()部分将在本章的第二部分解释。
...
![9781430250920_Fig05-01.jpg](Images/9781430250920_Fig05-01.jpg)

[图 5-1](#_Fig1) 。地址的两种状态(折叠和展开)

![image](Images/sq.jpg) **注意**你可能在过去遇到过使用 element.style.display='block '作为 none 反义词的脚本。这适用于大多数元素，但是简单地将显示值设置为 nothing 会将其重置为初始显示值——不一定是 block 它可以是内联的或表格行的。如果添加一个空字符串，就让浏览器来设置适当的值；否则，你必须为不同的元素添加一个开关块或 if 条件。
...
![image](Images/sq.jpg) **注意**在 https://developer.mozilla.org/en-US/docs/CSS/CSS_Reference 的 Mozilla 开发者网络上有一个包含厂商前缀 CSS 的参考指南。
...
![image](Images/sq.jpg) **注**在本章的后面你会听到列表中的前两项。然而，您在这里找不到动画或拖放示例，因为这些是高级 JavaScript 主题，需要大量解释，超出了本书的范围。你会在第 11 章中找到现成的例子。
...
![image](Images/sq.jpg) **注意**从逻辑上讲，你也应该能够通过 setAttribute()方法改变类，但是浏览器对此的支持是不可靠的(例如，Internet Explorer 不允许 class 或 style 作为属性)，这就是为什么目前坚持使用 className 是一个好计划。属性的名称是 className，而不是 class，因为 class 是 JavaScript 中的保留字，当用作属性时会导致错误。
...
![image](Images/sq.jpg) **注意** DOMhelp 已经包含了 cssjs()方法；因此，您不需要将其包含在此示例中。
...
![9781430250920_Fig05-02.jpg](Images/9781430250920_Fig05-02.jpg)
...
![9781430250920_Fig05-03.jpg](Images/9781430250920_Fig05-03.jpg)
...
![image](Images/sq.jpg) **注意**您需要在这里将最高参数重置为 0，因为 getHighest()需要查找作为参数发送的元素中的最高值，而不是您修复的所有元素中的最高值。如果出于某种反常的意外，H3 高于最高的段落，你会在段落和“更多”链接之间产生间隙。
...
![image](Images/sq.jpg) **注意**你需要把最高的参数变成一个数字，加上一个 *px* 后缀，然后再应用到元素的高度。情况总是如此；当涉及到元素的 CSS 尺寸时，你不能简单地分配一个没有单位的数字。
...
![9781430250920_Fig05-04.jpg](Images/9781430250920_Fig05-04.jpg)
...
![9781430250920_Fig05-05.jpg](Images/9781430250920_Fig05-05.jpg)
...
![image](Images/sq.jpg) **注意**想象一个事件处理程序，比如一个运动探测器或者门铃的触点——如果有人靠近门，灯就会打开；如果有人按下门铃的按钮，电路闭合，触发响铃机构发出声音。同样，您可以检测用户何时将鼠标悬停在链接上以触发一个功能，以及当用户单击该链接时触发另一个功能。
...
![image](Images/sq.jpg) **注意**你不需要测试 ID 为 info 的元素，因为函数只能被它调用。
...
![9781430250920_Fig05-06.jpg](Images/9781430250920_Fig05-06.jpg)
...
![image](Images/sq.jpg) **注意**这里的 helper 方法已经包含在 DOMhelp.js 里面了，如果想不用 DOMhelp 也能在那里找到。
...
![image](Images/sq.jpg) **注意** Safari 在 5.1 之前的版本中支持 stopPropagation()，但没有任何作用。在 5.1 及更高版本中，此问题已得到修复。
...
![image](Images/sq.jpg) **注**HL . onclick = DOM help . safariclickfix；可能是一个简单的 HL . onclick = function(){ return false；};然而，一旦 Safari 开发团队解决了这个问题，搜索和替换这个修复将变得更加容易。
...
![9781430250920_Fig05-07.jpg](Images/9781430250920_Fig05-07.jpg)
...
![image](Images/sq.jpg) **注意**通常，在每个 keyup 事件中检查字段内容是否是一个数字就足够了，但是这展示了键盘事件的强大功能。
