...
最后，要运行应用，您需要将脸书的 app_secret 密钥放入 AndroidManifest.xml 文件，如[Figure 8–6](#fig_8_6)所示。
...
在模拟器上将 FacebookPluginExample 作为 Android 应用运行。第一个屏幕将显示脸书登录页面，如[图 8–7](#fig_8_7)所示。
...
1.  Register C2DMPlug-in
    ...
    在 plugins.xml 文件中添加以下 XML 元素作为“plugins”元素的子元素，如[Figure 8–11](#fig_8_11)所示。您可能需要在“res”文件夹下创建一个 xml 文件夹，并从 PhoneGap Android 示例应用中复制 plugins.xml。
...
2.  Include the native part of the plug-in into the project
    ...
    从 C2DM-plug-in 文件夹中复制 src 文件夹，如[图 8–12](#fig_8_12)所示，并将其粘贴到我们的 PhoneGap 应用的根目录中，即“MobilePushPluginExample”。
...
3.  Include the JavaScript part of the plug-in into the project
    ...
    如[Figure 8–13](#fig_8_13)所示，从 C2DM-plug-in 文件夹中复制以下文件，并将其粘贴到我们应用的 assets 文件夹中。
...
2.  Move `C2DMReceiver.java`
    ...
    我们必须将 C2DMReceiver.java 从 com.phonegap.c2dm 包中移到我们的应用项目中，即 org . examples . mobile push plugin . example，为此，将 C2DMReceiver.java 从 com.phonegap.c2dm 包中拖放到 org . examples . mobile push plugin . example 中，如[Figure 8–15](#fig_8_15)所示。
...
前往`[http://code.google.com/android/c2dm/signup](http://code.google.com/android/c2dm/signup)`并填写表格以注册发送者。你必须提到 Android 应用的包名和发件人帐户电子邮件以及其他信息。对于我们的应用，包名应该是“org . examples . mobilepushplugin . example ”,我们应该使用 Google 帐户作为发件人帐户电子邮件。我们必须在 PhoneGap 应用中使用发件人帐户电子邮件 id 来注册接收通知的设备。
...
您需要使用目标为“Google API(Google Inc .)-API Level 8”的 AVD (Android 虚拟设备)来运行支持 C2DM 的 Android 应用。请参考[第 2 章](02.html#ch2)从“Android SDK 和 AVD 管理器”创建新的 AVD。
...
我们将在模拟器上看到我们的应用收到的通知，如[图 8–19](#fig_8_19)所示。

![images](img/9781430239031_Fig08-19.jpg)

**图 8–19。** *模拟器上的推送消息*

如果您想利用 iPhone-PhoneGap 应用的推送通知服务，您可以使用 PhoneGap 插件，该插件位于:`[https://github.com/urbanairship/ios-phonegap-plugin](https://github.com/urbanairship/ios-phonegap-plugin)`。它使用城市飞艇服务来推送移动通知。
